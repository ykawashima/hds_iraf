...
begin
	...
	int xsize, ysize
	int x, y, dp
	real ave_cnt, cnt[21], resi[21], diff_cnt
	string ave1, ave2
	int i1, i2, i
	string pix_list
	string img1[21], img2[21]
	real bp
	real    temp1,temp2,temp3,temp4,temp5,temp6,center[100]
	int j, jmax
	int fw
	bool ans
...
ave1='tmp_ave'//e_1//'_CCD'//det_id//'_'//xbin//'x'//ybin
ave2='tmp_ave'//e_2//'_CCD'//det_id//'_'//xbin//'x'//ybin
...
ave1='ave'//e_1//'_CCD'//det_id//'_'//xbin//'x'//ybin//'-'//obsdate
if(access(ave1//'.fits')){
...
ave2='ave'//e_2//'_CCD'//det_id//'_'//xbin//'x'//ybin//'-'//obsdate
if(access(ave2//'.fits')){
        printf('>>> Averaged Flat %s already exists.\n', ave2
        printf('Do you want to delete it? <y/n> : ')
        while(scan(ans)==0) {}
...
diff='diff'//e_2//'-'//e_1//'_CCD'//det_id//'_'//xbin//'x'//ybin//'-'//obsdate
if(access(diff//'.fits')){
...
for(ix=dx;ix<xsize;ix=ix+dx){
    if((ix<1100/xbin)||(ix>1120/xbin)){
      for(iy=dy;iy<ysize;iy=iy+dy){
        listpix(ave2//'['//ix//','//iy//']',wcs='physical') \
	  | scan(temp1, ave_cnt)
        if((ave_cnt>low) && (ave_cnt<up)){
          listpix(diff//'['//ix//','//iy//']',wcs='physical') \
	    | scan(temp1, diff_cnt)
