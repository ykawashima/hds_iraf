...
#
#                      A.Tajitsu          2003.01.17
#------------------------------------------------------------
procedure hdslswave( inlist,outlist,complist )
#
...
retryid:
	identify( comp,section='middle line',databas='database',\
		coordli='linelists$thar.dat',nsum=20,match=20.,\
...
while( fscan( list, comp) == 1 )
{
	...
	reidentify(comp,comp,section='middle line',interac-,\
 		...
 		step=2,nsum=2,shift=0.,nlost=5,cradius=4.,\
...
joinlines(objlist2,outlist_full,output=obj_com_out,delim=' ')
...
while( fscan( list, obj, out) == 2 )
        {
        if(access(out//'.imh')||access(out//'.fits'))
	        {
...
retry:
        if(access('sky.imh')) imdel('sky.imh')
        if(access('sky.fits')) imdel('sky.fits')
	...
	if(access('database/fcsky')) del('database/fcsky')
        ...
        if(!fc_fl)
        {
	  if(access('database/fc'//comp)) del('database/fc'//comp)
          fitcoords(comp,\
	        fitname=comp,databas='database',interac+,\
...
trans:
	...
	transform(obj,out,fitname=comp,databas='database',\
