procedure sm_cont(imfile,smfile,st_x,ed_x,st_y,ed_y)
...
int x_st,x_ed,y_st,y_ed,i_log,sm_x,sm_y,sm_lev,f_oasis
...
#task	$pix2sm = ("$"//osfn("hdslshome$")//"contour_sm")
task	$pix2sm = ("$"//"contour_sm")
...
listpixels(iname//'['//x_st//':'//x_st+1//',*]',\
           wcs='world',formats='%g %g',ver-,mode=mode) | scan(w_st,tmp1,tmp2)
if(w_st > 10000)
{
    f_oasis = 1
    w_st = w_st/10000
...
listpixels(iname//'['//x_ed//':'//x_ed+1//',*]',\
           wcs='world',formats='%g %g',ver-,mode=mode) | scan(w_ed,tmp1,tmp2)
...
#printf('location 5000 29000 %d 25500\n', 25500-(y_ed-y_st)*300, >> sm_file//'.sm')
printf('location 5000 29000 5000 25500\n', >> sm_file//'.sm')
printf('box 1 2\n', >> sm_file//'.sm')
printf('levels lev\n', >> sm_file//'.sm')
printf('contour\n', >> sm_file//'.sm')
printf('image delete\n', >> sm_file//'.sm')
printf('define TeX_strings 1\n', >> sm_file//'.sm')
printf('expand 0.8\n', >> sm_file//'.sm')
printf('ylabel {\\bf Slit Position[arcsec]}\n', >> sm_file//'.sm')
if(f_oasis == 1)
{
    printf('xlabel {\\bf Wavelength[\\mu m]}\n', >> sm_file//'.sm')
}
else
{
    printf('xlabel {\\bf Wavelength[\\AA]}\n', >> sm_file//'.sm')
}
printf('expand 0.9\n', >> sm_file//'.sm')
printf('relocate %f %f\n',w_st,real(sm_y)*(s_pix/1.5), >> sm_file//'.sm')
